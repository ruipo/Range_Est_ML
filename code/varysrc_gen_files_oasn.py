...
for ii in range(src_xcoor.shape[0]): # for each source location
    for jj in range(20): # create 20 small random perturbations to the location
        ...
        for kk in range(layers.shape[0]):
            if kk != roughness_layer-1:
                ...
                outFile.write(line(layers[kk,0:-1]) + ' ' + line(roughness_param) + '\n')
        ...
        if src.src[3] != 0: #if there is discrete source  
            disc_src = np.array([src_zcoor,src_xcoor[ii]+random.uniform(-0.25, 0.25),src_ycoor,src_level]) # create random variation in ssp between [-0.25 0.25]
